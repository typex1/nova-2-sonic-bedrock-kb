...
export class NovaSonicBidirectionalStreamClient {
  ...
  private async processToolUse(toolName: string, toolUseContent: object): Promise<Object> {
    ...
    switch (tool) {
      case "ask_kiro":
        ...
        const question = this.parseKiroToolContent(toolUseContent);
        if (!question) {
          throw new Error('Could not parse question from tool content');
        }
        return this.askKiro(question);
      default:
  ...
  private parseKiroToolContent(toolUseContent: any): string | null {
    try {
      if (toolUseContent && typeof toolUseContent.content === 'string') {
      ...
      console.error("Failed to parse Kiro tool content:", error);
      return null;
    }
  }

  private async askKiro(question: string): Promise<Object> {
    ...
    return new Promise((resolve) => {
      execFile(
        ...
        (err, stdout, stderr) => {
          ...
          if (err) {
            console.error(`[kiro] Error: ${err.message}`);
            resolve({ error: err.message });
            return;
          }
          // Strip ANSI escape codes
          const clean = stdout.replace(/\x1B\[[0-9;]*[A-Za-z]/g, "").trim();
          console.log(`[kiro] Response: ${clean.slice(0, 200)}`);
